...
export function isPreview(req: Request | null): boolean {
  // @ts-ignore
...
export async function getPrismicApi(req: Request | null) {
...
export async function getDocuments(
  ...
  const docs: PrismicApiSearchResponse = await api.query(
    predicates.concat([Prismic.Predicates.not('document.tags', ['delist'])]), // uncomment this and comment out the line above to show delisted content
    // predicates,
    opts
  );
