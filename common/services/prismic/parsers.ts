...
import { CaptionedImage } from '../../model/captioned-image';
import { ImagePromo } from '../../model/image-promo';
import { GenericContentFields } from '../../model/generic-content-fields';
import { LabelField } from '../../model/label-field';
import { SameAs } from '../../model/same-as';
import { HtmlSerializer } from './html-serialisers';
...
function isEmptyHtmlString(maybeContent: HTMLString | null): boolean {
  return maybeContent ? asHtml(maybeContent) === null : false;
}

export function asText(maybeContent: HTMLString | null): string | null {
  return maybeContent && RichText.asText(maybeContent).trim();
}

export function asHtml(
  maybeContent: HTMLString | null,
...
export function parseRichText(maybeContent: HTMLString | null) {
...
export function checkAndParseImage(
  frag: PrismicFragment | null
): ImageType | null {
...
export function parseTaslFromString(pipedString: string): Tasl {
  ...
  try {
    ...
    const license: LicenseType | null = licenseTypeArray.find(
      l => l === maybeLicense
    ) as LicenseType;
    return {
      title,
...
function parseStructuredText(maybeFragment: any | null): HTMLString | null {
  return maybeFragment && isStructuredText(maybeFragment as HTMLString)
    ? (maybeFragment as HTMLString)
    : null;
}
...
export function isDocumentLink(fragment: PrismicFragment | null): boolean {
