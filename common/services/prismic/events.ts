import { UiEvent, EventFormat, EventTime } from '../../model/events';
import {
  PrismicDocument,
  PaginatedResults,
  PrismicQueryOpts,
...
function parseEventFormat(frag: any): EventFormat | null {
...
function parseEventBookingType(eventDoc: PrismicDocument): string | null {
...
export function parseEventDoc(
  ...
  scheduleDocs: PrismicApiSearchResponse | null = null
): UiEvent {
  ...
  const times =
    (data.times &&
      data.times // Annoyingly prismic puts blanks in here
  ...
  return { ...event, type: 'events', labels };
}
...
type EventsQueryProps = PrismicQueryOpts & {
  // @ts-ignore
...
function getNextDateInFuture(event: UiEvent): EventTime | null {
  const now = london();
