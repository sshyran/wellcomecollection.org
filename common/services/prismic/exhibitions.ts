...
import { PrismicFragment, PrismicDocument, PaginatedResults } from './types';
import {
  UiExhibition,
  UiExhibit,
  ExhibitionFormat,
...
export function parseExhibitionFormat(frag: any): ExhibitionFormat | null {
  ...
  return document
    .map(exhibit => {
      if (exhibit.item.type === 'exhibitions' && !exhibit.item.isBroken) {
        return {
          exhibitType: 'exhibitions' as const,
...
export async function getExhibitionExhibits(
  req: Request,
  { ids }: ExhibitQuery
): Promise<PaginatedResults<UiExhibit> | null> {
